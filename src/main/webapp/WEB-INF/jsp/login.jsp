			...
			<div class="${cssGroup}">
				<c:if test="${not empty param.login_error}">
					<div id="log-in-error" class="message error">
						Your login attempt was not successful, try again.<br /> <br /> Reason:
						<c:out value="${SPRING_SECURITY_LAST_EXCEPTION.message}" />
					</div>
				</c:if>

				<label for="j_username">Login</label> <input type="text" name="j_username" id="j_username"
					value='<c:if test="${not empty param.login_error}">
				<c:out value="${SPRING_SECURITY_LAST_EXCEPTION.authentication.principal}"/></c:if>' />


				<label for="j_password">Hasło</label> <input type="password" name="j_password" id="j_password" /><br />
